...
def make_parser():
    ...
    :return:
    """
    parser = argparse.ArgumentParser("Detection Demo!")
    ...
    parser.add_argument("--output_dir",
                        type=str,
                        default="/scratch/morrill/users/ie93/ByteTrack/YOLOX_outputs",
                        help="")
    parser.add_argument("--conf",
                        default=0.01,   #orignal 0.1
                        type=float,
                        help="test conf")
    ...
    parser.add_argument("-f",
                        "--exp_file",
                        default="/scratch/morrill/users/ie93/ByteTrack/YOLOX_outputs/yolox_x_ablation/100Train_16Test/testConf_0.01_nms_0.5/YOLOX_X/yolox_x_ablation.py",
                        type=str,
    ...
    parser.add_argument("--video_dir",
                        type=str,
                        default="/scratch/morrill/users/ie93/ByteTrack/videos",
                        help="")

    ## "--path", default="./datasets/mot/train/MOT17-05-FRCNN/img1", help="path to images or video"
    parser.add_argument("--path",
                        default="/scratch/morrill/users/ie93/ByteTrack/videos/761901019_cam4.mp4",
        ...
        if ret_val:
            with torch.no_grad():
                ...
                if outputs[0] is None:
                    ...
                    logger.info(f"Frame {frame_id}: detections = {len(outputs[0])}")
...
def imageflow_demo(predictor, vis_dir, current_time, args):
    ...
    if args.demo == "videos":
        ...
        if os.path.isfile(args.path):
            ...
            current_time = time.localtime()
            current_time = time.strftime("%Y_%m_%d_%H_%M_%S", current_time)
            save_path = os.path.join(save_dir, current_time + ".avi")
...
def run(exp, opt):
    ...
    if opt.save_result:
        vis_dir = os.path.join(file_name, "detect_vis")
