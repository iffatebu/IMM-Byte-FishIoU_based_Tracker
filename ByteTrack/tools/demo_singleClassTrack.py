...
def make_parser():
    ...
    parser.add_argument("-f",
                        "--exp_file",
                        default="/scratch/morrill/users/ie93/ByteTrack/YOLOX_outputs/yolox_x_ablation/100Train_16Test/testConf_0.01_nms_0.6/YOLOX_X/yolox_x_ablation.py", #?????? check if it is possible not to change"
                        type=str,
    ...
    parser.add_argument("--path",
                        #default="./videos/SC4-camera3_04-05-21_23-05-44.000.avi",
                        default= "./images/sample_test6/761901329_cam1_2/img1", #@@@@@@@@@@@@@  needed to update @@@@@@@@@@
...
def image_demo(predictor, vis_folder, result_root, path, current_time, save_result):
    ...
    seq_name = os.path.basename(os.path.dirname(path))
    ...
    for image_name in files:
        ...
        if save_result:
            ...
            save_folder = os.path.join(vis_folder, seq_name)
    ...
    base = os.path.splitext(os.path.basename(opt.path if opt.demo == "video" else seq_name))[0]
...
def run(exp, opt):
    ...
    if opt.save_result:
        vis_dir = os.path.join(file_name, "100Train_16Test/testConf_0.01_nms_0.6/Vis_floders") # @@@@@@@@@@ Change here visual Folder name @@@@@@@@@@@@
        os.makedirs(vis_dir, exist_ok=True)
        
        # NEW: root folder for all demo txt outputs
        result_root = os.path.join(file_name, "100Train_16Test/testConf_0.01_nms_0.6/result_files") # @@@@@@@@@@ Change here text Folder name @@@@@@@@@@@@
