# -*- coding: utf-8 -*-
"""
Script: Observation-Centric & Velocity direction with customized FishIoU matching.py
"""
...
# >>>>>>>new add: the direction cost
def direction_cost(tracks, detections, frame_id, lambda_=0.2, stale_max=5):
    """OC-SORT OCM as an additive soft penalty (observation-centric).
    0 when a detection lies along the track's recent heading, up to lambda_
    when it lies against it. Add to an IoU cost matrix. Never rewards, so it
    can only push wrong-direction matches away."""
    ...
    age = np.array([frame_id - t.last_obs_frame for t in tracks])           # (N,)
    enough = np.array([len(t.obs_history) >= 2 for t in tracks])
    has_dir = (np.linalg.norm(vel, axis=1) > 1e-6) & (age <= stale_max) & enough

    # reference = each track's OLDEST observation in the window (~DELTA_T ago)
    ref = np.array([(t.obs_history[0] if len(t.obs_history) else t.to_xyah()[:2])
                    for t in tracks], dtype=np.float64)                      # (N,2)
    det_c = np.array([d.to_xyah()[:2] for d in detections], dtype=np.float64)# (M,2)

    diff = det_c[None, :, :] - ref[:, None, :]                              # (N,M,2)
    norm = np.linalg.norm(diff, axis=2, keepdims=True) + 1e-6
    unit = diff / norm
    cosang = np.clip((unit * vel[:, None, :]).sum(axis=2), -1.0, 1.0)        # (N,M)
    dcost = (1.0 - cosang) / 2.0          # 0 aligned, 0.5 perp, 1 opposite
    dcost[~has_dir, :] = 0.0              # new / stationary / stale tracks: no penalty
    n_active = int(has_dir.sum())
    mean_pen = float(dcost[has_dir].mean()) if n_active else 0.0
    print(f"[DIR] frame={frame_id} tracks={N} with_heading={int(has_dir.sum())} "
      f"maxvel={np.linalg.norm(vel,axis=1).max() if N else 0:.2f} "
      f"maxpen={float(dcost.max()) if dcost.size else 0:.3f}")
    return lambda_ * dcost

# >>>>>>>new add: the direction cost: The has_dir mask is the safety valve for exactly the slow-milling 41/48 case you saw: when a fish 
# barely moves (tiny velocity), or its heading is stale because it was occluded behind a bar, the penalty switches off rather than 
# acting on a noisy heading. So it helps on fast crossings and stays quiet when it can't be trusted.
